...
public class RentalService : IRentalService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // if (rental.Units <= units && rental.PreparationTimeInDays >= preparationTimeInDays) - can update rental without checks
        if (rental.Units > units || rental.PreparationTimeInDays < preparationTimeInDays)
        {
        ...
        var newRental = new Rental(rental.Id, units, preparationTimeInDays, rental.Version + 1);
        try
        {
            await _rentalRepository.UpdateAsync(newRental, cancellationToken);
        }
        catch (DBConcurrencyException e)
        {
            _logger.UpdateRentalAsyncDbConcurrencyExceptionWasThrown(rental.Id, units, preparationTimeInDays, e.Message);
            return UpdateRentalResult.ConcurrencyException(e.Message);
        }
        return UpdateRentalResult.Successful(newRental);
    }
}